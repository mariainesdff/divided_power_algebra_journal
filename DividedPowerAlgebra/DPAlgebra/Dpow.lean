...
## TODO
* Prove that `DividedPowerAlgebra.Free.basis_grade` and `DividedPowerAlgebra.Free.basis` are
  indeed bases.
...
theorem dpow_eq (x : DividedPowerAlgebra R M) :
  ...
  classical
  have hmem : ∀ (n : ℕ), ∀ x ∈ augIdeal R M, ∃ y, ∃ (_ : n ≠ 0 → y ∈ augIdeal R M),
    (LinearMap.lift (S R) (f b)) y = hSN.dpow n ((LinearMap.lift (S R) (f b)) x) :=
    (fun n x hx ↦ ⟨dpow b n x, fun hn ↦ dpow_mem b hn hx, lift_dpow_eq_dpow_lift _ _ hx⟩)
  simp only [CharZero.dividedPowers, ofInjective]
  split_ifs with hx
  · simp only [RingHom.coe_coe, RatAlgebra.dpow_apply, ne_eq, exists_prop,
      ← (lift_injective b).eq_iff]
    rw [lift_dpow_eq_dpow_lift _ _ hx, ← (hmem n x hx).choose_spec.choose_spec]
    simp only [RatAlgebra.dpow_apply, ne_eq, exists_prop]
  · rw [dpow_null b hx]
...
theorem isSubDPIdeal_of_surjective (hL : DividedPowers (augIdeal R L))
    (dpow_eq_dp : ∀ (n : ℕ) (x : L), hL.dpow n (ι R L x) = dp R n x) (hf : Function.Surjective f) :
    hL.IsSubDPIdeal (RingHom.ker (LinearMap.lift R f)) := by
  rw [LinearMap.ker_lift_of_surjective hf, kerLift, hL.span_isSubDPIdeal_iff]
...
theorem isSubDPIdeal :
  ...
  exact ⟨(isSubDPIdeal_of_surjective R (Free.dividedPowers p.b)
    (Free.dpow_eq_dp p.b) p.surj).dpow_mem n hn hx.1, dpow_mem _ hn hx.2⟩

/-- The divided power structure on `augIdeal R M` associated with `p : Presentation R M`. -/
