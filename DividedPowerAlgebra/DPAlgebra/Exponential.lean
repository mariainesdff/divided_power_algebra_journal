...
We also prove Proposition IV.8 of [Roby1963], which describes the kernel of the surjective map
`DividedPowerAlgebra R M →ₐ[R] DividedPowerAlgebra R M/P`, where `P` is a submodule of `M`.
...
* `DividedPowerAlgebra.LinearMap.ker_lift_of_surjective`: the kernel of the canonical map
  `DividedPowerAlgebra R M →ₐ[R] DividedPowerAlgebra R N` associated with a surjective linear map
  `f : M →ₗ[R] N` is the ideal generated by the elements of the form`dp R n p`, for `n : PNat`
  and `p ∈ ker f`.  This is [Roby1963, Proposition IV.8].
...
/-- The algebra map `DividedPowerAlgebra R M ⧸ (kerLift f) →ₐ[R] DividedPowerAlgebra R N`
  induced by a surjective linear map `f : M →ₗ[R] N`. -/
noncomputable def quotientAlgHom :
...
@[simp]
theorem quotientAlgHom_mk_dp (n : ℕ) (m : M) :
    (quotientAlgHom f) ((Ideal.Quotient.mk (kerLift f)) (dp R n m)) = dp R n (f m) := by
  simp [quotientAlgHom, LinearMap.lift_apply_dp]

/-- The inverse of the map `DividedPowerAlgebra.quotientAlgHom`. -/
noncomputable def quotientAlgHom_inv :
...
@[simp]
lemma quotientAlgHom_inv_dp (k : ℕ) (m : M) :
    quotientAlgHom_inv f hf (dp R k (f m)) = Submodule.mkQ _ (dp R k m) := by
  simp [quotientAlgHom_inv, exponentialModule_equiv_symm_apply, coeff_linearMap,
    coeff_exp_LinearMap]
...
noncomputable def quotientAlgEquiv :
    (DividedPowerAlgebra R M ⧸ (kerLift f)) ≃ₐ[R] DividedPowerAlgebra R N := by
  apply AlgEquiv.ofAlgHom (quotientAlgHom f) (quotientAlgHom_inv f hf)
  · rw [algHom_ext_iff]
    intro k n
    obtain ⟨m, rfl⟩ := hf n
    simp
...
/-- : The kernel of the canonical map `DividedPowerAlgebra R M →ₐ[R] DividedPowerAlgebra R N`
  associated with a surjective linear map `f : M →ₗ[R] N` is the ideal generated by the elements of
  the form`dp R n p`, for `n : PNat` and `p ∈ ker f`.  This is [Roby1963, Proposition IV.8]. -/
theorem LinearMap.ker_lift_of_surjective (hf : Function.Surjective f) :
  ...
  simp [← EmbeddingLike.map_eq_zero_iff (f := (quotientAlgEquiv f hf)),
    quotientAlgEquiv, quotientAlgHom]
