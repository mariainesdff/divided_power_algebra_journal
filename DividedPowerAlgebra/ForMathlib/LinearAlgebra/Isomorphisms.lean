...
variable (P) in
/-- Given a surjective linear map `f : M →ₗ[R] N` and an `R`-module `P`, linear maps `N →ₗ[R] P`
  are in 1-1 correspondence with linear maps `g : M →ₗ[R] P` such that `ker f ⊆ ker g`. -/
