...
theorem toFun_zero_of_constantBiCoeff_zero (hf : biCoeff (0 : M × M') f 0 = 0) (S : Type*)
  ...
  · simp only [Prod.fst_zero, pow_zero, Prod.snd_zero, mul_one, hf, tmul_zero]
  · intro k hk hk0
...
theorem isBiHomogeneous_biCoeff_support_le {a b : ℕ} (hf : f.IsBiHomogeneous (a, b)) (m : M × M') :
...
theorem isBiHomogeneous_biCoeff_eq {a b : ℕ}
  ...
  rw [this, toFun_add_tmul_eq_biCoeff_sum, map_finsuppSum,
    sum_of_support_subset _ (isBiHomogeneous_biCoeff_support_le hf m) _ (by simp)]
  simp
