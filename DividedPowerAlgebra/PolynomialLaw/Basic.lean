...
* `LinearMap.toPolynomialLaw` : the `R`-linear map sending a linear map `v : M →ₗ[R] N` to the
 polynomial law given by base change of `v`.
...
theorem toFun'_eq_of_inclusion {A : Type u} [CommSemiring A] [Algebra R A] {φ : A →ₐ[R] S}
    {B : Type u} [CommSemiring B] [Algebra R B] {ψ : B →ₐ[R] S} (p : A ⊗[R] M) (q : B ⊗[R] M)
