...
theorem toFun_biGenerize_eq : f.toFun (MvPolynomial (Fin 2) R) (biGenerize m) =
  ...
  simp only [biGenerize_eq_generize, Nat.succ_eq_add_one, Nat.reduceAdd, toFun_generize_eq,
    finTwoArrowEquiv'_symm_apply', Finsupp.sum, biCoeff_apply_eq_coeff_apply]
  apply Finset.sum_bij (fun n _ ↦ finTwoArrowEquiv' ℕ n) _ _ _
    (fun _ _ ↦ by simp [Finsupp.ofSupportFinite_fin_two_eq])
  · intro n hn
    simpa [biCoeff_apply_eq_coeff_apply, Finsupp.ofSupportFinite_fin_two_eq,
      Finsupp.equivFunOnFinite.symm_apply_two] using hn
  · intro _ _ _ _ h
...
theorem toFun_add_tmul_eq_biCoeff_sum (S : Type*) [CommSemiring S] [Algebra R S] (r s : S) :
  ...
  simp only [Fin.isValue, Function.comp_apply, map_add, rTensor_tmul, AlgHom.toLinearMap_apply,
    aeval_X] at this
  simp only [finTwoArrowEquiv', Equiv.symm_trans_apply, finTwoArrowEquiv_symm_apply,
    Finsupp.coe_equivFunOnFinite_symm, Fin.isValue, Finsupp.equivFunOnFinite_symm_apply_toFun,
...
@[simp]
theorem Finsupp.equivFunOnFinite_symm_apply {α} [Finite α] (f : α →₀ M) (a : α) :
    Finsupp.equivFunOnFinite.symm f a = f a := by
...
theorem biGenerize_eq_sum_biCoeffBaseChange :
  ...
  · intro a ha
    simpa [biCoeffBaseChange, biCoeffBaseChange', Finsupp.ofSupportFinite_coe,
      Finsupp.ofSupportFinite_fin_two_eq, Finsupp.equivFunOnFinite.symm_apply_two] using ha
  · intro _ _ _ _ h
    simpa [Finsupp.ext_iff, Fin.forall_fin_two] using h
  · intro n hn
  ...
  · intro n hn
    simp [Finsupp.equivFunOnFinite.symm_apply_two, biCoeffBaseChange, biCoeffBaseChange',
